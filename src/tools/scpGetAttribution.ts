...
export async function scpGetAttributionToolCall(repo: ScpRepository, input: ScpGetAttributionToolInput) {
  const res = await repo.getAttribution({ link: input.link });
  return {
    attribution_text: res.attribution_text,
    authors: res.authors,
    license: SCP_CONTENT_LICENSE,
    attribution: buildPageAttribution({ url: res.page.url, title: res.page.title, authors: res.authors }),
  };
}
