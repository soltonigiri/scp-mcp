...
- 本文ファイルの取得方法はコレクションごとの公開形式に従う。
  - Items / Tales: `content_index.json` から本文ファイル名を解決する。
  - GOI: `content_goi.json` を使用する。
  - Hubs: `index.json` 内の本文を使用する。
- 本文ファイル名にパスが含まれる場合はbasenameだけを使用し、安全なJSONファイル名として検証する。
...
2. サーバのREADME/メタ情報として「非公式」「ライセンス遵守が必要」を明示する。([scp-api][1])
...
- 各Toolは構造化出力schemaを公開し、読み取り専用・非破壊・外部データ参照であることをMCP annotationで示す。
- 本文取得結果には、対応する `scp://content/{link}` Resource Linkを含める。
...
- 入力：
  ...
  - `start_line`（任意、1始まり、既定1）
  - `max_lines`（任意、既定200、最大500）

- 出力：
  - `content`（指定形式）
  - `source`: `{url, title, page_id}`
  - `content_hash`（整形済み本文全体のSHA-256）
...
- 出力：
  ...
  - `authors`（SCPPERから取得できた作品作者。取得できない場合は空配列）
...
- 最低限：ページタイトル、slug、タグ、シリーズ、ページ作成者に対するキーワード検索
- フィルタ：タグ、シリーズ、作成日範囲、レーティング範囲
- スコアリング：BM25等のキーワード検索 + 追加でrating/新しさ補正（任意）
- 本文は検索結果から選択したページについてオンデマンド取得し、検索索引には保持しない。
...
- npm packageは `scp-mcp` executableを提供し、`npx scp-mcp` でstdioサーバーを起動できる。
- `package.json` の `mcpName` と `server.json` の `name` / `version` が一致し、公式MCP Registry schemaを満たす。
